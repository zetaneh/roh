...
def main():
    ...
    with st.expander('PDF files'):
        if os.path.isfile('pdfs.csv'):
            ...
            pdf = read_pdf()
            pdf = [f'<embed src="data:application/pdf;base64,{base64.b64encode(open(f, "rb").read()).decode("utf-8")}" width="100%" height="600px" type="application/pdf">' for f in pdf]
            df_pdfs = pd.DataFrame({'pdf':pdf})
            df_pdfs.to_csv('pdfs.csv',index=False)
        for p in df_pdfs['pdf']:
            st.markdown(displayPDF(p), unsafe_allow_html=True)
            
            
    with st.expander('Multiple'):
        nn = st.number_input('Number',min_value=1,value=7)
        power = st.number_input('Power',min_value=1,value = 10)
        multiple = st.number_input('Multiple',min_value=1,value=23)
        st.write(f'Mult= {[nn*i for i in range(1,multiple+1)]}')
        st.write(f'Power= {[nn**i for i in range(1,power+1)]}')
