...
def main():
    ...
    with st.expander('PDF files')
        pdf = read_pdf()
        for p in pdf:
            st.markdown(f'<p style="text-align: center; font-size: 20px; font-weight: bold;">{p}</p>', unsafe_allow_html=True)
    ...
    if st.button('مسح'):
        text_0 = ''
    text = st.tex:t_area('ادخل النص هنا', value=text_0, height=200)
    #suggestions
    import pandas as pd
    import os
