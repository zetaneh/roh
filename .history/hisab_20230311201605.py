...
def main():
    ...
    if st.button('carré magique'):
        ...
        st.write(f'(x,r) = ({x},{r})')
        # multiply the matrix by x; add r to last n elements consecutively
        matrix = np.vectorize(convert_number)(matrix*x)
        matrix[-1] = np.vectorize(convert_number)(matrix[-1]+r)
