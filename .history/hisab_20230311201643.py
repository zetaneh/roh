...
def main():
    ...
    if st.button('carré magique'):
        ...
        st.write(f'(x,r) = ({x},{r})')
        # multiply the matrix by x; add r to last n elements conseec        # niput Number N in order to get the sum of rows, columns and diagonals and reverse diagonals N
    st.markdown(f'<hr style="border: 2px solid #000000;">', unsafe_allow_html=True)
    if st.button('طالع'):
        jomal = hisab_sarir(t)
        st.write(f'جمل : {jomal}')
